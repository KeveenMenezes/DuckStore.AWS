...
The codebase is mid-evolution: it was migrated from a classic stack (PostgreSQL/Marten, EF Core, RabbitMQ/MassTransit, gRPC, Carter) to serverless AWS. Architectural decisions are recorded under `docs/adr/` (47+ ADRs, numbered sequentially) — **read the relevant ADR before changing cross-cutting infrastructure**. Foundational ones: ADR-0004 (EventBridge over MassTransit/RabbitMQ), ADR-0005 (removal of in-process domain events; CDC via DynamoDB Streams), ADR-0006 (React/Next.js as the primary SPA), ADR-0019 (module-oriented service structure + rule-based stream publishers — the pattern every service below follows). Most recent Accepted: ADR-0046 (Challenge points redeemed into a customer-scoped Pricing discount); ADR-0040 established the strategy-dispatch grouping pattern CatalogView follows. Use the `adr` skill to author/update ADRs.
...
- Every `*.Function` project (except `User.Function`, which stays a plain class library) deploys as a self-contained **Native AOT ZIP on `provided.al2023`** (arm64), not a container image (ADR-0042).
...
- **Pricing.Function** — owns product pricing and promotional campaigns (ADR-0026). `Campaigns` module (`CreateCampaign`/`EndCampaign` features, plus the `CartDiscountAllocation` domain service that allocates a per-product campaign discount proportionally against the cart's single billed price — ADR-0043), `Prices` module (consumes `ProductDeleted` for cleanup, publishes price changes via CDC, `GetInstallmentPlan`/`GetBasketInstallmentPlan` queries; `pricing-product-discounts-stream-publisher` publishes `ProductDiscountChangedEvent` off the `product-discounts` stream so a campaign write — which never touches `prices` — still reaches CatalogView, ADR-0044), `GatewayCosts` module (payment gateway cost tracking, ADR-0028), `CustomerDiscounts` module (mints a customer-scoped discount off Challenges' `PointsRedeemedEvent` via `pricing-points-redeemed-consumer`, burns it `Issued`→`Consumed` off `PaymentAuthorizedEvent` via `pricing-payment-authorized-consumer` — ADR-0046). DynamoDB: `prices`, `campaigns`, `product-discounts`, `gateway-costs`, `customer-discounts`, `pricing-processed-events`.
...
- **Challenges.Function** — server-side code-challenge grading and progress bounded context (ADR-0045). `Questions` module owns the question bank and answer key (`challenges` table, PK `QuestionId`/SK `PUBLIC`|`ANSWER`, sparse GSI1 so the `ANSWER` item is structurally unreachable via the public read path); `Progress` module owns per-player attempts, score, and point redemption (`challenge-progress`, PK `OwnerId`/SK `PROFILE`|`ATTEMPT#`|`REDEMPTION#`, streams enabled). `challenges-submit-answer`/`challenges-reveal-hint`/`challenges-redeem-points` are Lambda-backed AppSync mutations (ADR-0009 escalation — grading transactions/domain invariants); `challenges-progress-stream-publisher` is a rule-based CDC publisher (ADR-0019) emitting `ChallengeAnsweredEvent`/`PointsRedeemedEvent` off the `challenge-progress` stream. CDC-out only, like Review — no inbound consumers. Redemption debits points only; currency conversion is Pricing's alone (ADR-0046).
...
- **src/AppHost** — .NET Aspire AppHost. `Program.cs` is the composition root; per-service wiring lives in one `*Extensions.cs` per service (`BasketExtensions`, `CatalogExtensions`, `CatalogViewExtensions`, `ChallengesExtensions`, `OrderingExtensions`, `PaymentExtensions`, `PricingExtensions`, `ReviewExtensions`, `UserExtensions`, plus `ObservabilityExtensions`). This is the source of truth for **local** infrastructure, Lambda handler names, `WaitFor`/`WaitForCompletion` chains, and DynamoDB Streams sources — for the real-AWS equivalent see `infra/` below. Shared helper in `Extensions/Extensions.cs`: `WithAwsDevEnvironment()` (dummy AWS creds + region for local dev). Add new resources/functions here — this is the only local wiring path.
...
- **src/WebApps/Shopping.Web.SPA.React** — React/Next.js SPA, the customer-facing storefront, fronting AppSync through its own Next.js BFF. The BFF holds Cognito tokens server-side in a `duckstore-sessions` DynamoDB table and exposes only an opaque `__Host-sid` session cookie to the browser, with `lib/auth/session.ts` as the sole module permitted to touch auth state and centralized, expiry-driven token refresh (ADR-0041, amends ADR-0016). Uses `pnpm` (`pnpm dev` / `pnpm build` / `pnpm lint`). Deployed via SST/OpenNext, not the CDK below (ADR-0020). This directory is slated to move into its own Git repository, linked back into DuckStore as a **git submodule**.
