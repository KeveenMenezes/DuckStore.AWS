...
## Example Flow: User login triggering Service A → Service B

### 1️⃣ User Authentication

1. User authenticates via **Cognito** → receives **User JWT token**.

### 2️⃣ User → Service A (via API Gateway)
...
### 3️⃣ Service A → Service B (Internal Call)
...
#### 🔹 Asynchronous (SQS/RabbitMQ + KMS Envelope)
